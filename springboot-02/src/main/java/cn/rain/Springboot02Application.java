...
/**
 * spring boot的主配置类，也可以再增加配置类，spring boot - 02模块的配置类都在config目录下。
 *
 ...
 * 并且指定要加载的xml文件的位置，且该注解要标注在配置类上，如spring boot的启动类。
 * 如果不使用该注解，无法加载我们自己编写的xml配置文件。但是spring boot不推荐使用这种方式添加组件，
 *
 * @author 任伟
 */
